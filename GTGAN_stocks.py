...
def train(
    ...
    fname = here / 'stock_model/stock_pretrained'
    ...
    for step in range(1, max_steps+1):
        ...
        if step % args.log_time == 0:
            x = batch_generator(dataset, batch_size).to(device)
            z = torch.randn(batch_size, x.size(
        ...
        if step % 500 == 0:
            ...
            with torch.no_grad():
                ...
                train_coeffs = controldiffeq.natural_cubic_spline_coeffs(
                    time, x)
...
def main():
    ...
    num_layers = 3
    x_hidden = 48
    args.save_dir = here / args.save_dir
    ...
    if args.train == True:
        ...
        predictive_score = list()
        for tt in range(args.max_steps_metric):
            temp_pred = predictive_score_metrics2(generated_data1, generated_data2)
            predictive_score.append(temp_pred)
